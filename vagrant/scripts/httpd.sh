...
sed -e 's/^DocumentRoot "\/var\/www\/html"$/DocumentRoot "\/var\/eoxsmagnetism\/instance"/' -i /etc/httpd/conf/httpd.conf
sed -e 's/^<Directory "\/var\/www\/html">$/<Directory "\/var\/eoxsmagnetism\/instance">/' -i /etc/httpd/conf/httpd.conf

# Adjust server name in /etc/httpd/conf/httpd.conf
sed -e 's/^#ServerName www.example.com:80$/ServerName eoxsmagnetism-vagrant/' -i /etc/httpd/conf/httpd.conf
